...
// @return graph
//
SpecificationGraph* createSpecificationGraph(int size){
	SpecificationGraph* graph = (SpecificationGraph*) malloc(size*sizeof(SpecificationGraph));
	if(graph == NULL){
		fprintf(stderr, "Error: Unable to malloc memory.\n");
		exit(-1);
	}
...
void insertNode(SpecificationGraph *dependencyGraph, Node *newNode){
	int graphSize = dependencyGraph->size;
	dependencyGraph->nodes[graphSize] = newNode;
...
void getLowestNode(SpecificationGraph *graph, Node* node, char** orderedArray, int* arrayIndex){
	...
	for(int i = 0; i < node->numOfDependencies; i++){
		Node *dependencyNode = findNode(graph, node->dependencies[i]);
...
int isRelative(SpecificationGraph *graph, Node *currNode, char* element){
	...
	for(int i = 0; i < currNode->numOfDependencies; i++){
		Node *dependencyNode = findNode(graph, currNode->dependencies[i]);
		if(dependencyNode != NULL){
